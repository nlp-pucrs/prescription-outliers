...
def getPrescriptions(prescription, med_name):
    prescription = prescription[prescription['MEDICAMENTO'] == med_name]
    X = prescription[['dose_conv','freq_dia']].values.astype(float)
...
class GaussianMixtureOutlier():
    ...
    def gerunParameterSearcht_params(self):
        return self.pb
...
def evaluateMethods(X,Y, p_svm, p_cov, p_ift, p_lof, p_wpr, p_gmx, debug=True):
    classifiers = getClassifiers(p_cov[0])
...
def getOverdoseMedications(prescription): 
    ...
    MostMedicines = prescription[['REG. PACIENTE','MEDICAMENTO']].groupby(['MEDICAMENTO']).agg(['count'])
    MostMedicines = MostMedicines['REG. PACIENTE']
